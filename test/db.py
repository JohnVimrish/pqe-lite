...
def get_table_stats(conn: psycopg.Connection, table_name: str) -> dict[str, Any]:
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(_STATS_QUERY, (table_name,))
        row = cur.fetchone()
    if not row:
        logger.warning("no pg_stat_user_tables entry for %s (new/unused table?)", table_name)
...
def table_has_index(conn: psycopg.Connection, table_name: str) -> bool:
    ...
    """
    query = "SELECT COUNT(*) from pg_indexes WHERE tablename = %s"
    with conn.cursor() as cur:
...
def get_indexed_columns(conn: psycopg.Connection, table_name: str) -> set[str]:
    ...
    """
    with conn.cursor() as cur:
        cur.execute(_INDEXED_COLUMNS_QUERY, (table_name,))
        return {row[0] for row in cur.fetchall()}
...
def run_analyze(conn: psycopg.Connection, table_name: str) -> None:
    logger.info("running ANALYZE on %s", table_name)
    try:
        with conn.cursor() as cur:
        ...
        conn.rollback()
        logger.error("ANALYZE failed on %s: %s", table_name, exc)
